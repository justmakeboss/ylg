...
//创建一个新实例
function InitKitchen2Control() {
    return new Kitchen2Control();

}

function Kitchen2JsonToObj(element) {
    var obj = new Kitchen2Control();
    obj.ShowWay = element.ShowWay;

    obj.Item = new Array();
    if (element.Item != null && element.Item.length > 0) {
        for (var i = 0; i < element.Item.length; i++) {
            var m = new Kitchen2Item();
...
Kitchen2Control.prototype = new BaseControl();

//渲染HTML
Kitchen2Control.prototype.Html = function () {
...
//展示
Kitchen2Control.prototype.Show = function () {
    ...
    if (this.Item.length <= 0) {
        ...
        for (var i = 0; i <2; i++) {
            var controlItem = new Kitchen2Item();
            currentControl.Add(controlItem);
            targetView.find(".big:eq("+i+")").attr("data-id", controlItem.ID);
        }
...
//操作监听
Kitchen2Control.prototype.ReadyListen = function () {
...
//子项拖拽
Kitchen2Control.prototype.Draggable = function () {
    var currentControl = this;
...
//排序
Kitchen2Control.prototype.Sort = function () {
    var currentControl = this;
...
//
Kitchen2Control.prototype.Upload = function () {
...
Kitchen2Control.prototype.UploadFinish = function (imgUrl) {
    currentControl = this;
    ...
    page.Set(currentControl);
    var targetView = $("#MyBox").find("div[data-id='" + currentControl.ID + "']");
    targetView.find("div[data-id='" + item.ID + "']").find("img").attr("src", imgUrl);
}

//子项输入改变
Kitchen2Control.prototype.InputChange = function () {
    var currentControl = this;
    ...
    $("#ItemList").find("input[name='Link']").bind("blur", function (e) {
       ...
       // var targetView = $("#MyBox").find("div[data-id='" + currentControl.ID + "']");
       // targetView.find("div[data-id='" + item.ID + "']").find(".LinkTo").attr("href", navigateUrl);

    })
}

//跳转链接改变
Kitchen2Control.prototype.NavigateChange = function () {
    var currentControl = this;
