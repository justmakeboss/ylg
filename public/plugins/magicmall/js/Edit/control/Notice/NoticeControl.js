...
//创建一个新实例
function InitNoticeControl()
{
  return new NoticeControl();

}

function NoticeJsonToObj(element) {
    var obj = new NoticeControl();
...
NoticeControl.prototype = new BaseControl();

//渲染HTML
NoticeControl.prototype.Html = function () {
...
//展示
NoticeControl.prototype.Show = function () {
    ...
    if (this.Item.length <= 0) {
        ...
        currentControl.Add(noticeItem);
        page.Set(currentControl);
        targetView.find(".draggable1").attr("data-id", noticeItem.ID);
    }
...
//操作监听
NoticeControl.prototype.ReadyListen = function () {
...
//显示编辑面板
NoticeControl.prototype.tabItem = function () {

    var currentControl = this;
    $('.diy-ctrl').find(".tabType").bind("click",function (e) {
        var noticeItem = new CarouselItem();
        var tab = $(this).attr('data-type');
        if(tab > 0){
            $("#ItemList").fadeIn(400);
            $(".addbtn-block").fadeIn(400);
            $("#noticeNum").hide();
        }else{
            $("#noticeNum").fadeIn(400);
            $("#ItemList").fadeOut(400);
            $(".addbtn-block").fadeOut(400);
        }
    });
...
//显示方式动作
NoticeControl.prototype.ExtendToShowWay = function ()
{
   // this.Height = 100;
    var currentControl = this;
...
//显示编辑面板
NoticeControl.prototype.ExtendToAddItem = function () {
    ...
    $('.diy-ctrl').find(".addbtn-block").bind("click",function (e) {
        ...
        page.Set(currentControl);
        var html = juicer($("#" + currentControl.Type + "Item-template").html(), { Model: noticeItem, DataModel: DataModel });
        ...
        if (currentControl.Item.length > 1)
        {
            ...
            var htmlLi = juicer($("#" + currentControl.Type + "ListControl").html(), { Model: noticeItem });
            targetView.find(".picList").append(htmlLi);
        } else {
            targetView.find(".picList").find("li:first").attr("data-id", noticeItem.ID);
...
//移除某一个项
NoticeControl.prototype.Delete = function () {
    var currentControl = this;
...
//子项拖拽
NoticeControl.prototype.Draggable = function () {
    var currentControl = this;
...
NoticeControl.prototype.Sort = function () {
    var currentControl = this;
    ...
    if (currentControl.Item.length > 0) {
        ...
        $("#ItemList").children("li").each(function (index, element) {
            ...
            var noticeItem = currentControl.Get(id);


            if (noticeItem != undefined && noticeItem != null) {
                newArray.push(noticeItem);
            }
        });
...
//
NoticeControl.prototype.Upload = function () {
    ...
    $(".configurator").delegate(".openFile", "click", function (e) {
        ...
        $.UploadingnoticeItem.UploadingnoticeItem = item;
        $.UploadingnoticeItem.UploadingControl = currentControl;
        $.UploadingnoticeItem.li = $li;

    })

}
NoticeControl.prototype.UploadFinish = function (imgUrl) {
    currentControl = this;
    var item = $.UploadingnoticeItem.UploadingnoticeItem;
    item.Img = imgUrl;
...
//跳转链接改变
NoticeControl.prototype.NavigateChange = function () {
    var currentControl = this;
...
//子项输入改变
NoticeControl.prototype.InputChange = function () {
    var currentControl = this;
