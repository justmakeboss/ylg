...
class Controller
{
     ...
     * 获取模板赋值所有变量
     * */
    protected function viewAssign($filter = [])
    {
        $defalut_filter = ['action','template_now_time'];

        //['goods'=>['goods_id']]
        $arr_filter = !$filter ? $defalut_filter : array_merge($defalut_filter,$filter);

        $viewassign = $this->view->viewAssign();
        //过滤字段
        foreach($viewassign as $key => $value)
        {
            if(in_array($key,$arr_filter))
                unset($viewassign[$key]);
        }
   ...
   /* //深度处理过滤字段
    function filter($filter,$data)
    {
        ...
        {
            if(!is_array($v) && in_array($v,array_keys($data)))
                unset($data[$k]);

            $this->filter($v,$data[$k]);
        }
