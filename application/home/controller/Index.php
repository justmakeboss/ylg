...
//        	}
        ...
        //活动区
        $list = M('goods')
            ->where('type_id=6 and status=1 and is_on_sale=1 AND UNIX_TIMESTAMP(agentend_time) <= UNIX_TIMESTAMP(NOW()) AND UNIX_TIMESTAMP(saleend_time) > UNIX_TIMESTAMP(NOW())')
            ->field(['goods_id', 'goods_name', 'shop_price', 'is_recommend','is_hot','original_img'])->order('sort asc')->select();

        //批发区
        $list2 = M('goods')
            ->where('type_id=6 and status=0 and is_on_sale=1 AND UNIX_TIMESTAMP(agentstart_time) <= UNIX_TIMESTAMP(NOW()) AND UNIX_TIMESTAMP(NOW()) <=  UNIX_TIMESTAMP(agentend_time)')
            ->field(['goods_id', 'goods_name', 'shop_price', 'is_recommend','is_hot','original_img,agentend_time'])->order('sort asc')->select();
        //普通专区
        $list3 = M('goods')->where('type_id=0 and is_on_sale=1')->field(['goods_id', 'is_recommend','is_hot', 'goods_name', 'shop_price','original_img'])->order('sort asc')->select();
