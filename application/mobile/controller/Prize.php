...
class Prize extends MobileBase
{
    // 注意提示，房车奖的金额是写死
    ...
    public function prize($id, $prize_info, $order_id = 0, $steps = 1,$goods = []){
        ...
        if (in_array($prize_info,$prize_user)) {
            if (!$id && !$prize_info) return ['code' => -1, 'msg' => '参数异常'];
        ...
        if (in_array($prize_info,$prize_user_verify)) {
            ...
            if (!in_array($user_info['level'],$identity_arr)) return ['code' => -1, 'msg' => '身份不符合'];
        }
        ...
        switch ($prize_info){
            ...
            case 'car_home_prize':
                if ($steps == 1) {
                    ...
                    exit(json_encode($result['config'],JSON_UNESCAPED_UNICODE));
    ...
    private  function prize_switch($prize_name,$prize_info, $info = []){
        ...
        if ($config[$prize_info.'_'.'switch']) {
            ...
            if ($prize_info == 'market_prize') {
                ...
                return ['code' => -1, 'msg' => '基金添加成功'];
            } 

        } else {
            // 开关关闭
            return ['code' => -1, 'msg' => '该奖项已关闭!', 'config' => $config];
        }
    }
    ...
    private function first_prize($first_info, $config, $prize_info){
        ...
        // 对用户佣金分发以及记录
        if (empty($user_id) && !is_int($user_id)) return(['code' => -1, 'msg' => '异常']);
    ...
    private function market_prize($config){
        ...
        if ($distribut) {
            ...
            if ($time_unit < time()) return ['code' => -1 , 'msg' => '尚未到达分奖时间'];
        } else {
            $prize_begin_time = Db::name('distribut_system')->where(['inc_type' => 'settlement', 'name' =>  'prize_begin_time'])->value('value');
            if (!$config['market_prize_time_begin']) return  ['code' => -1 , 'msg' => '尚未设置分奖时间'];
            //无分奖记录 首次分奖  
            switch ($unit) {
                case 1:
        ...
        $funds_money = Db::name('distribut_system')
                        ->where(['inc_type' => 'settlement', 'name' => 'funds'])
                        ->value('value');
        if ($funds_money <= 0 ) return ['code' => -1 , 'msg' => '奖池为零'];
        // 获取参与分奖的会员
        ...
        if (!$user_num) return ['code'=>-1,'msg'=> '无参与人员信息'];
        $user_prize_money = round($funds_money / $user_num, 2);
        if ($user_prize_money <= 0 ) return ['code' => -1 , 'msg' => '奖励过低'];
        foreach ($user_arr as $key => $value) {
        ...
        if ($res) return ['code'=> 1, 'msg' => '本次补助完毕'];
        return ['code'=> -1, 'msg' => '本次补助完毕,奖池异常', 'data' => $res];
    ...
    private function discount_prize($user_id,$config, $goods){
        ...
        switch ($config['discount_prize_goods_status']) {
            case 0:  // 只算普通商品
                ...
                break;
            case 1: // 只算身份产品
                if ($goods['type_id']) $goods['shop_price'] * $config['discount'] / 100;
    ...
    private function general_prize($user_info, $config, $order_id){
        ...
        switch ($len) {
            ...
            case 3:
                ...
                break;    
            default:
                return ['code' => -1, 'msg' => '无层级信息'];
                break;
        }
        return ['code' => 1, 'msg' => '奖项已处理'];
    }
