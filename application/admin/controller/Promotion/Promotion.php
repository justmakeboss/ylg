...
//        $data['group'] = (empty($data['group'])) ? '' : implode(',', $data['group']); //前台暂时不用这个功能，先注释
    ...
    public function prom_goods_del()
    {
        ...
        $this->success('删除活动成功', U('Promotion.Promotion/prom_goods_list'));
    }
...
//        }
    ...
    public function prom_order_save()
    {
        ...
        $this->success('编辑促销活动成功', U('Promotion.Promotion/prom_order_list'));
    }
    ...
    public function prom_order_del()
    {
        ...
        $this->success('删除活动成功', U('Promotion.Promotion/prom_order_list'));
    }
...
//        dump($where);exit;
    ...
    public function flash_sale_infos()
    {
        if(IS_POST){
            $data = I('post.');
            if($data['del_id']){
                $list = Db::name('flash_sale')->where("id = {$data['del_id']}")->find();
                if($list){
              Db::name('goods')->where("goods_id", $list['goods_id'])->save(array('prom_id'=>0,'prom_type' => 0));
                    $this->ajaxReturn(['status' => 1, 'msg' => '显示自营产品成功', 'result' => '']);
                }
            }else{
                $this->ajaxReturn(['status' => 1, 'msg' => '选择有效商品', 'result' => '']);
            }
        }
   ...
   /* public function flash_sale_del()
    ...
    {
        $this->assign("URL_upload", U('Admin/Ueditor.Ueditor/imageUp', array('savepath' => 'promotion')));
        $this->assign("URL_fileUp", U('Admin/Ueditor.Ueditor/fileUp', array('savepath' => 'promotion')));
        $this->assign("URL_scrawlUp", U('Admin/Ueditor.Ueditor/scrawlUp', array('savepath' => 'promotion')));
        $this->assign("URL_getRemoteImage", U('Admin/Ueditor.Ueditor/getRemoteImage', array('savepath' => 'promotion')));
        $this->assign("URL_imageManager", U('Admin/Ueditor.Ueditor/imageManager', array('savepath' => 'promotion')));
        $this->assign("URL_imageUp", U('Admin/Ueditor.Ueditor/imageUp', array('savepath' => 'promotion')));
        $this->assign("URL_getMovie", U('Admin/Ueditor.Ueditor/getMovie', array('savepath' => 'promotion')));
    ...
    public function flash_sale_del()
    {
        $id = I('del_id/d');
        if ($id) {
            $flash_sale = Db::name('flash_sale')->where(['id' => $id])->find();
            $spec_goods = Db::name('spec_goods_price')->where(['prom_type' => 1, 'prom_id' => $flash_sale['activity_id'],'item_id'=>$flash_sale['item_id']])->find();
            //有活动商品规格
            if($spec_goods){
                Db::name('spec_goods_price')->where(['prom_type' => 1, 'item_id' => $spec_goods['item_id']])->save(array('prom_id' => 0, 'prom_type' => 0));
                //商品下的规格是否都没有活动
                $goods_spec_num = Db::name('spec_goods_price')->where(['prom_type' => 1, 'goods_id' => $spec_goods['goods_id']])->find();
                if(empty($goods_spec_num)){
                    //商品下的规格都没有活动,把商品回复普通商品
                ...
                //没有商品规格
                Db::name('goods')->where(['prom_type' => 1, 'goods_id' => $flash_sale['goods_id']])->save(array('prom_id' => 0, 'prom_type' => 0));
            }
            M('flash_sale')->where(['id' => $id])->delete();
            exit(json_encode(1));
        } else {
