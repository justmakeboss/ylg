...
class Wechat extends Base
{
    ...
    public function index(){
        ...
        header("Location:".U('Wechat.Wechat/setting',['id'=>$wx_user['id']]));
        exit;
    }
    ...
    public function add(){
        ...
        if(IS_POST){
            ...
            if($row){
                $this->success('添加成功',U('Admin/Wechat.Wechat/setting',array('id'=>$row)));
            }else{
                $this->error('操作失败');
            }
            exit;
    ...
    public function setting()
    {
        ...
        if(empty($wechat)){
            return $this->error('请先在公众号配置添加公众号，才能进行微信菜单管理', U('Admin/Wechat.Wechat/index'));
        }
        if(IS_POST){
    ...
    public function menu(){
        ...
        if(empty($wechat)){
            $this->error('请先在公众号配置添加公众号，才能进行微信菜单管理', U('Admin/Wechat.Wechat/index'));
        }
        if(IS_POST){
            ...
            $this->success('操作成功,进入发布步骤',U('Admin/Wechat.Wechat/pub_menu'));
            exit;
        }
        //获取最大ID
    ...
    public function pub_menu(){
        ...
        // http post请求
        if(!count($p_menus) > 0){
           $this->error('没有菜单可发布',U('Wechat.Wechat/menu'));
            exit;
        }
        $access_token = $this->get_access_token($wechat['appid'],$wechat['appsecret']);
        if(!$access_token){
            $this->error('获取access_token失败',U('Wechat.Wechat/menu')); //  http://www.tpshop.com/index.php/Admin/Wechat.Wechat/menu
...
//        exit($post_str);
        ...
        if($return['errcode'] == 0){
            $this->success('菜单已成功生成',U('Wechat.Wechat/menu'));
        }else{
    ...
    public function text(){
        ...
        $wechat = M('wx_user')->find();
        if(empty($wechat)){
            $this->error('请先在公众号配置添加公众号，才能进行文本回复管理', U('Admin/Wechat.Wechat/index'));
        }
    ...
    {
        $id = input('id/d');
        $type = input('type', WxReply::TYPE_FOLLOW);
        $types = $this->auto_reply_menu();
        if (!key_exists($type, $types)) {
            $this->error("标签 $type 不存在");
        }
    ...
    public function add_text(){
        $wechat = M('wx_user')->find();
        if(empty($wechat)){
            $this->error('请先在公众号配置添加公众号，才能添加文本回复', U('Admin/Wechat.Wechat/index'));
        }
        if(IS_POST){
            $edit = I('get.edit');
            ...
            $row ? $this->success("添加成功",U('Admin/Wechat.Wechat/text')) : $this->error("添加失败",U('Admin/Wechat.Wechat/text'));
            exit;
        }
    ...
    public function img(){
        $wechat = M('wx_user')->find();
        if(empty($wechat)){
            $this->error('请先在公众号配置添加公众号，才能进行图文回复管理', U('Admin/Wechat.Wechat/index'));
        }
    ...
    public function add_img(){
        $wechat = M('wx_user')->find();
        if(empty($wechat)){
            $this->error('请先在公众号配置添加公众号，才能添加图文回复', U('Admin/Wechat.Wechat/index'));
        }
        if(IS_POST){
            ...
            $row ? $this->success("添加成功",U('Admin/Wechat.Wechat/img')) : $this->error("添加失败",U('Admin/Wechat.Wechat/img'));
            exit;
        }
    ...
    public function news(){
        $wechat = M('wx_user')->find();
        if(empty($wechat)){
            $this->error('请先在公众号配置添加公众号，才能进行多图文管理', U('Admin/Wechat.Wechat/index'));
        }
    ...
    public function add_news(){
        $wechat = M('wx_user')->find();
        if(empty($wechat)){
            $this->error('请先在公众号配置添加公众号，才能添加多图文', U('Admin/Wechat.Wechat/index'));
        }
        if(IS_POST){
            ...
            if($arr)
                array_pop($arr);
            if(count($arr) <= 1){
                $this->error("单图文请到图文回复设置",U('Admin/Wechat.Wechat/news'));
                exit;
            }
            $add['keyword'] =  I('post.keyword');
            $add['token'] =  $wechat['token'];
            ...
            //添加模式
                ...
                $add['type'] = 'NEWS';
                $row = M('wx_keyword')->add($add);
            $row ? $this->success("添加成功",U('Admin/Wechat.Wechat/news')) : $this->error("添加失败",U('Admin/Wechat.Wechat/news'));
            exit;
        }
        return $this->fetch();
    }
    ...
    public function fans_list()
    {
        ...
        $num = 10;
        $logic = new WechatLogic;
        $return = $logic->getUserFanList($p, $num, $keyword);
        if ($return['status'] != 1) {
    ...
    public function follow(){
        if(IS_POST){
            ...
            $this->success("操作成功",U('Wechat.Wechat/follow',array('inc_type'=>$inc_type)));
        }
        $config = tpCache("wechat_type");
