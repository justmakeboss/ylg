...
//        $row = $model->where($condition)->save($update);
    ...
    public function shipping_print(){
    	if(IS_POST){
    		...
    		if($r !== false){
    			$this->success('编辑成功',U('Plugins.Plugin/index')); exit;
    		}else{
    			$this->success('编辑失败',U('Plugins.Plugin/index'));
    		}
    	}
        ...
        if(IS_POST){
            ...
            if(I('get.edit') == 1){
                ...
                if($row){
                    ...
                    if(I('post.default') == 1){
                        //默认全国其他地区
                        exit($this->success("添加成功",U('Admin/Plugins.Plugin/shipping_list',array('type'=>'shipping','code'=>$add['shipping_code']))));
                    }
                    M('area_region')->insertAll($add2)&&exit($this->success("添加成功",U('Admin/Plugins.Plugin/shipping_list',array('type'=>'shipping','code'=>$add['shipping_code']))));
                }
    ...
    public function add_shipping()
    {
        ...
        $this->success("添加成功", U("Plugins.Plugin/index"));
    }
