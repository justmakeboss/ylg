...
class Goods extends Base {
            ...
            if(I('is_ajax') == 1)
            {
                ...
                if(!$validate->batch()->check(input('post.')))
                {
                    ...
                    $return_arr = array(
                        'status' => 1,
                        'msg'   => '操作成功',
                        'data'  => array('url'=>U('Admin/Goods.Goods/categoryList')),
                    );
                    $this->ajaxReturn($return_arr);
    ...
    public function delGoodsCategory(){
        ...
        $this->ajaxReturn(['status' => 1,'msg' =>'操作成功','url'=>U('Admin/Goods.Goods/categoryList')]);
    }
    ...
    public function ajaxGoodsList(){
        ...
        $count = M('Goods')
                ->alias('g')
                ->where($where)
                ->join('__TYPE_IDENTITY__ ti', 'g.type_id = ti.type_id','LEFT')
                ->count();
    ...
    public function addEditGoods()
    {
        ...
        if ((I('is_ajax') == 1) && IS_POST) {
            ...
            if(empty($data['type_id'])){
                ...
                if($data['cat_id'] === '0' || $data['cat_id_2'] === '0'){
                    $errors['cat_id'] = '商品分类必选';
//                    $this->ajaxReturns('商品分类必选');
                }

                if(empty($data['cost_price'])){
                    $errors['cost_price'] = '消费积分必填';
//                    $this->ajaxReturns('消费积分必填');
                }
//                if(empty($data['shop_price'])){
//                    $errors['shop_price'] = '本店零售价必填';
////                    $this->ajaxReturns('消费积分必填');
//                }

            }else{
                if(empty($data['name'][0]) || empty($data['trade_price'][0]) || empty($data['quota'][0])){
                    $return_arr = array(
                        'status' => -1,
                        'msg' => '代理产品套餐不能为空',
                        'data' => '代理产品套餐不能为空',
...
//                if(empty($data['shop_prices'])){
//                    $errors['shop_prices'] = '本店批发价必填';
////                    $this->ajaxReturns('本店批发价必填');
//                }
//                if(empty($data['market_price'])){
//                    $errors['market_price'] = '配额必填';
////                    $this->ajaxReturns('配额必填');
//                }
                if(empty($data['agentstart_time'])){
                    $errors['agentstart_time'] = '代理开始时间必填';
//                    $this->ajaxReturns('销售结束时间必填');
                }
                if(empty($data['saleend_time'])){
                    $errors['saleend_time'] = '销售结束时间必填';
//                    $this->ajaxReturns('销售结束时间必填');
                }
                if(empty($data['agentend_time'])){
                    $errors['agentend_time'] = '代理结束时间必填';
//                    $this->ajaxReturns('代理结束时间必填');
                }
            ...
            if(!empty($errors)){
                $error_msgs = array_values($errors);
                $return_arr = array(
                    'status' => -1,
                    'msg' => $error_msgs[0],
                    'data' => $errors,
...
//            $data['saleend_time'] = strtotime($data['saleend_time']);
//            $data['agentend_time'] = strtotime($data['agentend_time']);
//            $data['agentstart_time'] = strtotime($data['agentstart_time']);
            ...
            if ($type == 2) {
                if(!empty($data['name'])){
                    ...
                    foreach($data['name'] as $k=>$v){
                        $goodsdate['name'] = $v;
                        $goodsdate['id'] = $data['id'][$k];
//                        $goodsdate['price'] =  $data['price'][$k];
                        $goodsdate['trade_price'] =  floatval($data['trade_price'][$k]);
                        $goodsdate['quota'] =  intval($data['quota'][$k]);
                        $goodsdate['stock'] =  intval($data['stock'][$k]);
                        if(empty($goodsdate['id'])){
                            $goodsdate['goods_id'] = $goods_id;
                            Db::name('goods_setmeal')->add($goodsdate);
                        }else{
                            Db::name('goods_setmeal')->where('id',$goodsdate['id'])->update($goodsdate);
                        }
                    }
//                    $shopid = implode($data['id']);
//                    dump($goodsdate);exit;
                ...
                if(!empty($data['name'])){
                    ...
                    foreach($data['name'] as $k=>$v){
                        $goodsdate[$k]['name'] = $v;
                        $goodsdate[$k]['goods_id'] = $goods_id;
                        $goodsdate[$k]['trade_price'] =  floatval($data['trade_price'][$k]);
                        $goodsdate[$k]['quota'] =  intval($data['quota'][$k]);
                        $goodsdate[$k]['stock'] =  intval($data['stock'][$k]);
                    }
                    Db::name('goods_setmeal')->insertAll($goodsdate);
                }
            }
        ...
        $goods_setmeal = Db::name('goods_setmeal')->where('goods_id=' . I('GET.id', 0))->select();
        ...
        $GoodsLogic = new GoodsLogic();
        $type_info = $GoodsLogic->type_identity_list(['is_show' => 1]);
        $this->assign('type_info', $type_info);
        $this->assign('goods_setmeal', $goods_setmeal);
    ...
    public function addEditGoodsType()
    {
        ...
        if (IS_POST) {
            ...
            $this->success("操作成功!!!", U('Admin/Goods.Goods/goodsTypeList'));
            exit;
        }
                    ...
                    if(!$validate->batch()->check($post_data))
                    {
                        ...
                        $this->ajaxReturn($return_arr);
                             ...
                             $return_arr = array(
                                 'status' => 1,
                                 'msg'   => '操作成功',
                                 'data'  => array('url'=>U('Admin/Goods.Goods/goodsAttributeList')),
                             );
                             $this->ajaxReturn($return_arr);
                }
            }
    ...
    public function delGoods()
    {
        ...
        $this->ajaxReturn(['status' => 1,'msg' => '操作成功','url'=>U("Admin/Goods.Goods/goodsList")]);
    }
    ...
    public function delGoodsType()
    {
        ...
        $count > 0 && $this->error('该类型下有商品规格不得删除!',U('Admin/Goods.Goods/goodsTypeList'));
        // 判断 商品属性
        $count = M("GoodsAttribute")->where("type_id = {$id}")->count("1");
        $count > 0 && $this->error('该类型下有商品属性不得删除!',U('Admin/Goods.Goods/goodsTypeList'));
        // 删除分类
    ...
    public function delGoodsAttribute()
    {
        ...
        $this->ajaxReturn(['status' => 1,'msg' => "操作成功!",'url'=>U('Admin/Goods.Goods/goodsAttributeList')]);
    }
    ...
    public function delGoodsSpec()
    {
        ...
        $this->ajaxReturn(['status' => 1,'msg' => "操作成功!!!",'url'=>U('Admin/Goods.Goods/specList')]);
    }
    ...
    public function delBrand()
    {
        ...
        if($res){
            $this->ajaxReturn(['status' => 1,'msg' => '操作成功','url'=>U("Admin/goods.Goods/brandList")]);
        }
     ...
     * 身份产品列表
     * */
    public function type_identity_list(){
        $GoodsLogic = new GoodsLogic();
        $type_identity_list = $GoodsLogic->type_identity_list();
        $this->assign('type_identity_list',$type_identity_list);
        return $this->fetch();
    }
     ...
     * 身份产品 添加/修改
     * */
    public function addEditTypeIdentity(){
        if(IS_GET)
        {
            $type_identity = Db::name('type_identity')->where(['type_id' => I('GET.type_id',0)])->find();
            $this->assign('type_identity',$type_identity);
            return $this->fetch();
            exit;
        }
        $type_identity = Db::name('type_identity');
        $type = I('type_id') > 0 ? 2 : 1; // 标识自动验证时的 场景 1 表示插入 2 表示更新
        ...
        if(I('is_ajax') == 1)
        {
            ...
            if(!$validate->batch()->check($post_data))
            {
                ...
                //查找同级分类是否有重复分类
                $same_cate = $type_identity->where([ 'name'=>$post_data['name']])->find();
                if($same_cate){
                    $return_arr = array(
                        'status' => 0,
                        'msg' => '同级已有相同分类存在',
                ...
                if ($type == 2){
                    $insert_id = $type_identity->where(['type_id' =>$post_data['type_id']])->save($post_data); // 写入数据到数据库
                } else {
                    $post_data['add_time'] = time();
                    $insert_id = $type_identity->insertGetId($post_data);
                }
                if ($insert_id) {
                    $return_arr = array(
                        'status' => 1,
                        'msg'   => '操作成功',
                        'data'  => array('url'=>U('Admin/Goods.Goods/type_identity_list')),
                    );
                } else {
                    $return_arr = array(
                        'status' => -1,
    ...
    //删除 商品套餐表
    public function ajaxsetmeal(){
       $id = input(id);
        Db::name('goods_setmeal')->where('id',$id)->delete();
    }
}