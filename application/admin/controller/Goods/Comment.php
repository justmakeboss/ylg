...
class Comment extends Base {
    ...
    public function commentHandle(){
        ...
        if($row !== false){
            $this->ajaxReturn(['status' => 1,'msg' => '操作完成','url'=>U('Admin/Goods.Comment/index')]);
        }else{
            $this->ajaxReturn(['status' => -1,'msg' => '操作失败','url'=>U('Admin/Goods.Comment/index')]);
        }
    }
    ...
    public function ask_handle()
    {
        ...
        if($row !== false){
            $this->ajaxReturn(['status' => 1,'msg' => '操作完成','url'=>U('Admin/Goods.Comment/ask_list')]);
        }else{
            $this->ajaxReturn(['status' => -1,'msg' => '操作失败','url'=>U('Admin/Goods.Comment/ask_list')]);
        }
    }
}