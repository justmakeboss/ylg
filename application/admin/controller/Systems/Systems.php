...
class Systems extends Base
{
	...
	public function handle()
	{
		...
		$this->success("操作成功",U('Systems.Systems/index',array('inc_type'=>$inc_type)));
	}
    ...
    public function addEditNav()
    {
        ...
        if (IS_POST) {
            ...
            $this->success("操作成功!!!", U('Admin/Systems.Systems/navigationList'));
            exit;
        }
        // 点击过来编辑时
        $id = I('id',0);
...
//		tpCache($param['inc_type'],$param); //注释掉，不注释会出现重复写入数据库
    ...
    {
        $control = I('controller');
        $planPath = APP_PATH.'admin/controller/'.$control;
        $diffArray = array();
     ...
     * 选择控制器后找出该控制器下所有方法
     */
    function ajax_get_action2()
    {
        $control = I('controller');
        $menu = I('menu'); // 选中的菜单
        $advContrl = get_class_methods("app\\admin\\controller\\".$menu."\\".str_replace('.php','',$control));
        $baseContrl = get_class_methods('app\admin\controller\Base');
        $diffArray  = array_diff($advContrl,$baseContrl);
        $html = '';
        foreach ($diffArray as $val){
     	...
     	if(IS_POST){
     		...
     		if(!empty($data['id'])){
     			...
     			if(M('system_menu')->where(array('name'=>$data['name']))->count()>0){
     				$this->error('该权限名称已添加，请检查',U('Systems.Systems/right_list'));
     			}
     			unset($data['id']);
     			M('system_menu')->add($data);
     		}
     		$this->success('操作成功',U('Systems.Systems/right_list'));
     		exit;
     	}
     	$id = I('id');
     	if($id){
    ...
    public function app_management(){
        if(IS_POST){
            ...
            $data = I('post.');
            //图片上传类
            $android_link = $this->files_upload();
            $app_management = db::name('management')->where(array('type'=>$data['type']))->find();

            $data['android_link'] = $android_link ? $android_link : false;
            if(!$data['android_link']){
                $this->error("安装包上传失败",U('Systems.Systems/app_management',array('id'=>$data['id'])));
            }else{
                $result = db::name('management')->where(array('id'=>$app_management['id']))->save(array('android_link'=>""));
            }

;
            if($app_management) {
                $result = db::name('management')->where(array('id'=>$app_management['id']))->save($data);
            }else{
                $result = db::name('management')->save($data);
            }

            if($result)$this->success("操作成功",U('Systems.Systems/app_management',array('id'=>$data['id'])));
            $this->error("操作失败",U('Systems.Systems/app_management',array('id'=>$data['id'])));
        }

        $app_info = db::name('management')->where(array('type'=>1))->find();
        $apiurl = 'http://'.$_SERVER['HTTP_HOST'].'/index.php?m=mobile&c=Download&a=app_description';
    ...
    public function files_upload(){
        ...
        if($_FILES["android_link"]["error"]) {
           echo '文件上传错误';
        }else {

            //控制上传的文件类型，大小
            if($_FILES["android_link"]["type"]=="application/vnd.android.package-archive" && $_FILES["android_link"]["size"]<50000000)
            {
                ...
                $filePath = "public/upload/installpackage/";
                //判断文件是否存在
                if (!($_exists = file_exists($filePath))){
                    $isMk = mkdir($filePath,0777,true);
                }

                    //保存文件
                    //move_uploaded_file是php自带的函数，前面是旧的路径，后面是新的路径
                   $result = move_uploaded_file($_FILES["android_link"]["tmp_name"],$filePath.$filename);
