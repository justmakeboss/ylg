...
class Admin extends Base
{
    ...
    public function modify_pwd()
    {
        ...
        if (IS_POST) {
            ...
            if(session('suppliers_id')){
                $suppliers = M('suppliers')->where('suppliers_id', session('suppliers_id'))->find();
                if (!$suppliers || $suppliers['password'] != $enOldPwd) {
                    ...
                    $row = M('suppliers')->where('suppliers_id', session('suppliers_id'))->save(array('password' => $enNewPwd));
                    if ($row) {
    ...
    public function adminHandle()
    {
        ...
        if ($data['act'] == 'add') {
            ...
            if (D('admin')->where("user_name", $data['user_name'])->count()) {
                $this->error("此用户名已被注册，请更换", U('Admin/Systems.Admin/admin_info'));
            } else {
                $r = D('admin')->add($data);
            }
        }
        ...
        if ($r) {
            $this->success("操作成功", U('Admin/Systems.Admin/index'));
        } else {
            $this->error("操作失败", U('Admin/Systems.Admin/index'));
        }
    }
     ...
     * 门店登录入口
     * */
    public function suppliers_login(){
        if (session('?suppliers_id') && session('suppliers_id') > 0 || session('?admin_id') && session('admin_id') > 0) {
            $this->error("您已登录", U('Admin/Index/index'));
        }
        ...
        if (IS_POST) {
            ...
            if (!empty($condition['account']) && !empty($condition['password'])) {
                $condition['password'] = encrypt($condition['password']);
                $admin_info = M('suppliers')->join(PREFIX . 'admin_role', PREFIX . 'suppliers.role_id=' . PREFIX . 'admin_role.role_id', 'INNER')->where($condition)->find();
                if (is_array($admin_info)) {
                    //TODO 判断当前登录门店状态
                    if($admin_info['is_check']){
                        session('suppliers_id', $admin_info['suppliers_id']);
                        session('act_list', $admin_info['act_list']);
                        M('suppliers')->where("suppliers_id = " . $admin_info['suppliers_id'])->save(array('last_login' => time(), 'last_ip' => request()->ip()));
                        session('last_login_time', $admin_info['last_login']);
                        session('last_login_ip', $admin_info['last_ip']);
                        adminLog('门店后台登录');
                        $url = session('from_url') ? session('from_url') : U('Admin/Index/index');
                        ...
                        exit(json_encode(array('status' => 0, 'msg' => '账号已被禁止或关闭，请联系超级管理员')));
                    }
                } else {
                    exit(json_encode(array('status' => 0, 'msg' => '账号密码不正确')));
                }
    ...
    {
        $url = session('?suppliers_id') ? U('Admin/Systems.Admin/suppliers_login') : U('Admin/Systems.Admin/login');
        session_unset();
        session_destroy();
        session::clear();
    ...
    public function roleSave()
    {
        ...
        if ($r) {
            adminLog('管理角色');
            $this->success("操作成功!", U('Admin/Systems.Admin/role_info', array('role_id' => $data['role_id'])));
        } else {
            $this->error("操作失败!", U('Admin/Systems.Admin/role'));
        }
    }
    ...
    public function supplierHandle()
    {
        ...
        if ($data['act'] == 'add') {
            ...
            if ($count) {
                $this->error("此供应商名称已被注册，请更换", U('Admin/Systems.Admin/supplier_info'));
            } else {
        ...
        if ($r !== false) {
            $this->success("操作成功", U('Admin/Systems.Admin/supplier'));
        } else {
            $this->error("操作失败", U('Admin/SystemsAdmin/supplier'));
        }
    }
    ...
    public function delDate(){
        ...
        $tabName = input('post.tabName');
        $idName = input('post.idName');
        $id = input('post.id');
        $isDelete = input('post.isDelete',0);
        if ($isDelete) {
            $res = Db::name("$tabName")->where(["$idName" => $id])->save(['isDelete'=>1]);
        } else {
            $res = Db::name("$tabName")->where(["$idName" => $id])->delete();
        }
        if ($res) {
    ...
    /**
     * 更改system_menu表里的数据
     * eg:由 User@index改为User.User@index
     * 注:此方法只调用一次,不要重复调用
     * TIME:18-9-20 MEI
     */
    public function up_system_menu()
    {
		echo "1";die;
        $list = M('system_menu')->select();
        foreach ($list as $keyL => $valueL) {
            // right数据打散为数组
            $right = explode(',',$valueL['right']);
            foreach ($right as $key => $value) {
                // 取出@前面的字符串 eg:User
                $right_front = substr($value,0,strpos($value, '@'));
                // 拼接赋值 eg:User.User@index,
                $right[$key] = $right_front.'.'.$value.',';
            }
            // 数组再拼回字符串  
            $rightStrV = implode('', $right);
            // 去掉最尾部的','
            $rightStr = substr($rightStrV,0,strlen($rightStrV)-1);
            // 更改数据
            $update['right'] = $rightStr;
            M('system_menu')->where('id', $valueL['id'])->save($update);
        }
    }
}