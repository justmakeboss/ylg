	...
	public function add()
	{
		if (IS_POST) {
			...
			if (empty($pickup_id)) {
				...
				if ($add_res === false) {
					$this->error('添加失败', U('Admin/Systems.Pickup/add'));
				} else {
					$this->error('添加成功', U('Admin/Systems.Pickup/index'));
				}
			} else {
				//修改
				$update_res = $model->where(array('pickup_id' => $pickup_id))->save($data);
				if ($update_res === false) {
					$this->error('更新失败', U('Admin/Systems.Pickup/edit_address', array('pickup_id' => $pickup_id)));
				} else {
					$this->success('更新成功', U('Admin/Systems.Pickup/index'));
				}
			}
		}
