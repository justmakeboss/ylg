    ...
    public function addUser($user)
    {
        ...
        if(!empty($user['first_leader'])){
            $parent = Db::name("users")
                ->field("user_id,second_leader")
                ->where('mobile',$user['first_leader'])
                ->whereOr('user_id','=',$user['first_leader'])
                ->find();
            //填写的推荐人不存在
            if (empty($parent)) {
            ...
            if(!empty($parent['second_leader'])){
                $user['second_leader']=$parent['second_leader'].','.$parent['user_id'];
            }else{
                $user['second_leader']=$parent['user_id'];
            }
        }
