...
//        	$btn['delivery_confirm'] = '确认收货';
    ...
    public function deliveryHandle($data){
		...
		$delivery_status = M('order')->where("order_id=".$data['order_id'])->save($updata);//改变订单状态
		//发货成功
		if($delivery_status){
			//准备验证升级角色,此处是发货的地方----------标记<QualificationLogic>
			// $qualificationLogic = new \app\common\logic\QualificationLogic();
			// $qualificationLogic->prepare_update_level($data['user_id'],['order_id'=>$data['order_id']]);
		}
     ...
     *  更换服务订单表对应的状态标识
     */
    public function getServiceInfo($order){
        foreach ($order as $k => &$v) {
            $v['user_id'] = Db::name('users')->field('nickname,mobile')->where('user_id = '.$v['user_id'])->find();

            empty($v['plan_id']) ? :$v['plan_id'] = Db::name('repair_plan')->field('content')->where('id = '.$v['plan_id'])->find()['content'];
            empty($v['suppliers_id']) ? :$v['suppliers_id'] = Db::name('suppliers')->field('suppliers_name')->where('suppliers_id = '.$v['suppliers_id'])->find()['suppliers_name'];
            empty($v['engineer_id']) ? :$v['engineer_id'] = Db::name('users')->field('nickname')->where('user_id = '.$v['engineer_id'].' and is_engineer = 1 and engineer_status = 1')->find()['nickname'];
            empty($v['province']) ? :$v['province'] = Db::name('region')->field('name')->where('id = '.$v['province'])->find()['name'];
            empty($v['city']) ? :$v['city'] = Db::name('region')->field('name')->where('id = '.$v['city'])->find()['name'];
            empty($v['district']) ? :$v['district'] = Db::name('region')->field('name')->where('id = '.$v['district'])->find()['name'];

            $order_status_arr = ['接单中', '维修中', '已完成', '待评价', '已作废','已取消'];
            $order_type_arr = ['寄修', '上门', '到店'];
            $order_state_arr = ['商品安装', '维修服务'];
            $pay_status_arr = ['未支付', '已支付'];
            $v['order_status'] = $order_status_arr[$v['order_status']];
            $v['order_type'] = $order_type_arr[$v['order_type']];
            $v['order_state'] = $order_state_arr[$v['order_state']];
            $v['pay_status'] = $pay_status_arr[$v['pay_status']];

            $v['add_time'] = date('Y:m:d H:i:s', $v['add_time']);
            $v['pay_time'] = date('Y:m:d H:i:s', $v['pay_time']);
            $v['last_time'] = date('Y:m:d H:i:s', $v['last_time']);
            $v['start_server_time'] = date('Y:m:d H:i:s', $v['start_server_time']);
            $v['end_server_time'] = date('H:i:s', $v['end_server_time']);
