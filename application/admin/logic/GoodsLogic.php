...
class GoodsLogic extends Model
{
    ...
    public function repair_cat_list($ddb, $selected = 0, $re_type = true, $level = 0)
    {
                global $cat_category, $cat_category2;
                $sql = "SELECT * FROM  __PREFIX__$ddb ORDER BY parent_id , sort_order ASC";
    ...
    public function repair_cat_tree($id)
    {
        global $cat_category, $cat_category2;
    ...
    public function refresh_repaircat($id)
    {

        $GoodsCategory = M("repair_cat"); // 实例化User对象
        $cat = $GoodsCategory->where("id = $id")->find(); // 找出他自己
        // 刚新增的分类先把它的值重置一下

        if($cat['parent_id_path'] == '')
        {
            ($cat['parent_id'] == 0) && Db::execute("UPDATE __PREFIX__repair_cat set  parent_id_path = '0_$id', level = 1 where id = $id"); // 如果是一级分类
            Db::execute("UPDATE __PREFIX__repair_cat AS a ,__PREFIX__repair_cat AS b SET a.parent_id_path = CONCAT_WS('_',b.parent_id_path,'$id'),a.level = (b.level+1) WHERE a.parent_id=b.id AND a.id = $id");
            $cat = $GoodsCategory->where("id = $id")->find(); // 从新找出他自己
        }
        ...
        $time = time();
        $replace_str = $parent_cat['parent_id_path'].'_'.$id;
        Db::execute("UPDATE `__PREFIX__repair_cat` SET parent_id_path = REPLACE(parent_id_path,'{$cat['parent_id_path']}','$replace_str'), level = (level - $replace_level),add_time = $time  WHERE  parent_id_path LIKE '{$cat['parent_id_path']}%'");
    }

    public function refresh_repairspec($id)
    {

        $GoodsCategory = M("repair_spec"); // 实例化User对象
        $cat = $GoodsCategory->where("id = $id")->find(); // 找出他自己
        // 刚新增的分类先把它的值重置一下

        if($cat['parent_id_path'] == '')
        {
            ($cat['parent_id'] == 0) && Db::execute("UPDATE __PREFIX__repair_spec set  parent_id_path = '0_$id', level = 1 where id = $id"); // 如果是一级分类
            Db::execute("UPDATE __PREFIX__repair_spec AS a ,__PREFIX__repair_spec AS b SET a.parent_id_path = CONCAT_WS('_',b.parent_id_path,'$id'),a.level = (b.level+1) WHERE a.parent_id=b.id AND a.id = $id");
            $cat = $GoodsCategory->where("id = $id")->find(); // 从新找出他自己
        }
        ...
        $time = time();
        $replace_str = $parent_cat['parent_id_path'].'_'.$id;
        Db::execute("UPDATE `__PREFIX__repair_spec` SET parent_id_path = REPLACE(parent_id_path,'{$cat['parent_id_path']}','$replace_str'), level = (level - $replace_level),add_time = $time  WHERE  parent_id_path LIKE '{$cat['parent_id_path']}%'");
    }

    public function refresh_repairfault($id)
    {

        $GoodsCategory = M("repair_fault"); // 实例化User对象
        $cat = $GoodsCategory->where("id = $id")->find(); // 找出他自己
        // 刚新增的分类先把它的值重置一下

        if($cat['parent_id_path'] == '')
        {
            ($cat['parent_id'] == 0) && Db::execute("UPDATE __PREFIX__repair_fault set  parent_id_path = '0_$id', level = 1 where id = $id"); // 如果是一级分类
            Db::execute("UPDATE __PREFIX__repair_fault AS a ,__PREFIX__repair_fault AS b SET a.parent_id_path = CONCAT_WS('_',b.parent_id_path,'$id'),a.level = (b.level+1) WHERE a.parent_id=b.id AND a.id = $id");
            $cat = $GoodsCategory->where("id = $id")->find(); // 从新找出他自己
        }
        ...
        $time = time();
        $replace_str = $parent_cat['parent_id_path'].'_'.$id;
        Db::execute("UPDATE `__PREFIX__repair_fault` SET parent_id_path = REPLACE(parent_id_path,'{$cat['parent_id_path']}','$replace_str'), level = (level - $replace_level),add_time = $time  WHERE  parent_id_path LIKE '{$cat['parent_id_path']}%'");
    }
    ...
    function find_cat_parent_cat($cat_id,$ddb)
    {
        if($cat_id == null)
            return array();

        $cat_list =  M($ddb)->getField('id,parent_id,level');
        $cat_level_arr[$cat_list[$cat_id]['level']] = $cat_id;
     ...
     * 身份产品
     * */
    public function type_identity_list($where = []){
        return Db::name('type_identity')->where($where)->select();
    }
}