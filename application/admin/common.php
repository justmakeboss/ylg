 ...
 * @param $order_id 订单id
 */
function updateRetabeLog($order_id){
    //分销设置
    $maid_time = distributCache('settlement.maid_time');
    $rebate_status = $maid_time == 0 ? 2 : 1;//$rebate_status = 1 按确认收货规则，2 按订单支付分佣规则分佣
    $rebate_data = M('rebate_log')->where(array("order_id"=>$order_id,'status'=>3))->select();
    if($rebate_data){
        foreach($rebate_data as $key=>$val){
            accountLog($val['user_id'],-$val['money'],0,'用户申请退款佣金返还',-$val['money'],$val['order_id'],$val['order_sn']);
        }
    }
}
//订单商品售后退款
function updateRefundGoods($rec_id,$refund_type=0){
    ...
    updateRetabeLog($return_goods['order_id']);//佣金返还记录
}
...
function updateRefundOrder($order,$type=0){
    ...
    if($order['order_amount']>0 && $type == 1){
    	...
    	accountLog($order['user_id'],$order['order_amount'],0,'用户取消订单退款',$order['shop_integral'],$order['order_id'],$order['order_sn'],100);
    }
    //符合条件的，该笔订单使用的优惠券就退还
    ...
    updateRetabeLog($order['order_id']);//佣金返还记录
    return true;
}
...
function getSuppliersInfo($suppliers_id){
    $data = D('suppliers')->where("suppliers_id", $suppliers_id)->find();
    $data['user_name'] = $data['suppliers_name'];
    return $data;
}
...
function getMenuArrNew(){
    ...
    foreach ($list as $key => $value) {
        if (!empty($value['eng_name']) && $value['pid'] ==0) { // 有英文名字 为一级菜单
            ...
            $twoWhere = array('is_show'=>1,'is_select'=>1,'pid'=>$value['id']);
            $twoList = M('diyadmin_menu')->where($twoWhere)->select(); //二级菜单
           
            $two = array();
            foreach ($twoList as $twoKey => $twoValue) {

                $threeWhere = array('is_show'=>1,'is_select'=>1,'pid'=>$twoValue['id']);
                $threeList = M('diyadmin_menu')->where($threeWhere)->select(); //三级菜单
                $three = array();
                foreach ($threeList as $threeKey => $threeValue) {
                	if (!empty($threeValue['c'])) {
                		$three[] = array(
            ...
            $menuArr[$value['eng_name']] = array();
            $menuArr[$value['eng_name']]['name'] = $value['name']; // 一级名称赋值
            $menuArr[$value['eng_name']]['child'] = $two;
        }
    }
...
function memberLog($userId = 0, $beforeLevel,$afterLevel = 0, $desc = '', $type = 1){
    if (!$userId){return ;}
    ...
    return M('vpay_level_log')->add($data);
}