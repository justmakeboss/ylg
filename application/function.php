...
function check_mobile($mobile){
    if(preg_match('/1[3457869]\d{9}$/',$mobile))
        return true;
    return false;
}
...
/**
 * 银行信息
 * @param string $name 银行卡代码
 * @param string $type 需要查找的键值

 */
function bank_information($name="",$type=""){

    $bank_info = array(
        'ICBC' => array(
            'name'=>'中国工商银行',
            'image'=>'/template/mobile/new2/static/assets/images/icbc.png'
        ),
        'ABC' => array(
            'name'=>'中国农业银行',
            'image'=>'/template/mobile/new2/static/assets/images/abc.png'
        ),
        'CCB' => array(
            'name'=>'中国建设银行',
            'image'=>'/template/mobile/new2/static/assets/images/ccb.png'
        ),
        'BOC' => array(
            'name'=>'中国银行',
            'image'=>'/template/mobile/new2/static/assets/images/boc.png'
        ),
        'CMB' => array(
            'name'=>'招商银行',
            'image'=>'/template/mobile/new2/static/assets/images/cmb.png'
        ),
        'COMM' => array(
            'name'=>'交通银行',
            'image'=>'/template/mobile/new2/static/assets/images/ccom.png'
        ),
        'CITIC' => array(
            'name'=>'中信银行',
            'image'=>'/template/mobile/new2/static/assets/images/citic.png'
        ),
        'CIB' => array(
            'name'=>'兴业银行',
            'image'=>'/template/mobile/new2/static/assets/images/cib.png'
        ),
        'SPDB' => array(
            'name'=>'上海浦东发展银行',
            'image'=>'/template/mobile/new2/static/assets/images/spdb.png'
        )
    );

    if($name){
        foreach($bank_info as $k=>$vo){
            $result = strpos($vo['name'],$name) !== FALSE ? $vo : "";
            if($type && $result){
                $result = $vo[$type];
            }
...
/**
 * 二维数组根据键值去重复
 * @param array $arr 去重复的二维数组
 * @param string $key 需要查找的键值
 ...
 * 处理提交的图片
 * @param $file             //提交的文件
 * @param $file_addr        //存储的地址
 * @return string|boolean
 * Author:Faramita
 */
function deal_with_img($file,$file_addr){
    $image_upload_limit_size = config('image_upload_limit_size');
    $validate = ['size' => $image_upload_limit_size, 'ext' => 'jpg,png,gif,jpeg'];
    $dir = 'public/upload/'.$file_addr.'/';
    ...
    if ($info) {
        $img_address = '/' . $dir . $parentDir . '/' . $info->getFilename();
        return $img_address;
    } else {
        //上传错误提示错误信息
        return false;
