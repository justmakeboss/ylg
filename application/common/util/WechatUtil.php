...
class WechatUtil
{
    ...
    public function addTemplateMsg($template_sn)
    {
        ...
        $post = json_encode(['template_id_short' => $template_sn], JSON_UNESCAPED_UNICODE);
...
//            ]
     ...
     * 向一个粉丝发送消息
     */
    public function sendMsgToOne($openid, $type = 'text', $content)
    {
        $access_token = $this->getAccessToken();
        if (!$access_token) {
            return false;
     ...
     * @return boolean
     */
    public function sendMsgToMass($openids, $type = 'text',$content)
    {
        $access_token = $this->getAccessToken();
        if (!$access_token) {
            return false;
