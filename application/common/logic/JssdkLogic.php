  ...
  private function createNonceStr($length = 16) {
    ...
    public function send_template_message($order){
    	...
    	if(!empty($open_id)){
    		$tempalte = array(
    				'touser'=>$open_id,
    				'template_id'=>'EKmPbvt0OllOYTlKLvuicIS8OJOpR0tCxkBi6c5I9-Y',
     ...
     * @return JSON
     */
    public function send_template_toUser($open_id,$template_id, $url, $data, $topcolor = '#7B68EE'){
        $access_token = $this->get_access_token();
        //$open_id = M('users')->where(array('user_id'=>$user_id))->getField('openid');
        error_log($open_id.date('Ymd H:i:s'),3,'test.log');
        /*
        * data=>array(
        ...
        */
        if(!empty($open_id)){
            $tempalte = array(
                ...
                'url' => $url, //点击后跳转地址 SITE_URL.'/mobile'
                'topcolor'=>$topcolor,
                'data' => $data);
            $json_template = json_encode($tempalte);
            $url = "https://api.weixin.qq.com/cgi-bin/message/template/send?access_token={$access_token}";
