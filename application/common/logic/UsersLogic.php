...
class UsersLogic extends Model
{
    ...
    public function login($username,$password){
        ...
        }elseif($user['engineer_status'] != 1){
            $result = array('status'=>-3,'msg'=>'账号审核中！');
        }else{
            //查询用户信息之后, 查询用户的登记昵称
            $levelId = $user['level'];
...
//            M('users')->where(array('user_id' => $map['third_leader']))->setInc('underling_number');
    ...
    public function get_info($user_id)
    {
        ...
        $user['waitPay']     = M('order')->alias('o')->where("user_id = :user_id ".C('WAITPAY'))->bind(['user_id'=>$user_id])->count(); //待付款数量
        $user['waitSend']    = M('order')->alias('o')->where("user_id = :user_id ".C('WAITSEND'))->bind(['user_id'=>$user_id])->count(); //待发货数量
        $user['waitReceive'] = M('order')->alias('o')->where("user_id = :user_id ".C('WAITRECEIVE'))->bind(['user_id'=>$user_id])->count(); //待收货数量
        $user['waitEvaluation'] = M('order')->alias('o')->where("user_id = :user_id ".C('WAITCCOMMENT'))->bind(['user_id'=>$user_id])->count(); //待收货数量
     ...
     * @param int $type|明细标识 0:会员明细 1:工程师明细
     * @param string $starttime|开始时间戳，n天前时间戳
     * @param int $quantity|分页数量
     * @return mixed
     */
    public function get_account_log($user_id,$account_type = 0,$type = "",$starttime = "",$quantity = 16){
    ...
    /*修改用户积分或者账户*/
    public function setAccountOrJackpot($user_id,$type,$num,$other_info = [])
    {
        $user = Users::get($user_id);
        if($type =='account')
        {
    ...
    /*绑定上下级关系*/
    public function bindLeader($user_id,$parent_id, $perpetual = 0)
    {
        ...
        if ($perpetual) {
            ...
            $wechat->sendTemplateMsgBindLeader($parent_id,$user_id);
        }
        //他上线分销的下线人数要加1
        M('users')->where(array('user_id' => $map['first_leader']))->setInc('underling_number');
        M('users')->where(array('user_id' => $map['second_leader']))->setInc('underling_number');
     ...
     * 地址添加/编辑 申请记录  Lu
     * @param $user_id 用户id
     * @param $join_id 申请id(编辑时需传入)
     * $step 第几步
     * @return array
     */
    public function add_repair_join($user_id,$join_id=0,$type=0,$data,$step){
        $post = $data;
        $step = I('step');
        //检查手机格式
        if((!$post['province'] || !$post['city'] || !$post['district']) && $step <= 1)
            return array('status'=>-1,'msg'=>'所在地区不能为空','result'=>'');
        if(!$post['address'] && $step <= 1)
            return array('status'=>-1,'msg'=>'地址不能为空','result'=>'');
        if(!check_mobile($post['mobile']) && $step <= 1)
            return array('status'=>-1,'msg'=>'手机号码格式有误','result'=>'');
        if(!$post['repair_cat'] && $step == 2)
            return array('status'=>-1,'msg'=>'请选择维修品牌','result'=>'');
        if(!$post['repair_fault'] && $step == 2)
            return array('status'=>-1,'msg'=>'请选择维修故障','result'=>'');
        if(!$post['where_know'] && $step == 3)
            return array('status'=>-1,'msg'=>'请选择从哪里知道当日修','result'=>'');
        if($post['repair_cat']){
            $post['repair_cat'] = implode(",",$post['repair_cat']);
        }

        if($post['repair_fault']){
            $post['repair_fault'] = implode(",",$post['repair_fault']);
        }

        //编辑模式
        if($join_id > 0){

            $join = M('repair_join')->where(array('join_id'=>$join_id,'user_id'=> $user_id))->find();
            if ($step ==3)  $post['is_show'] = 1;
            $row=M('repair_join')->where(array('join_id'=>$join_id,'user_id'=>$user_id,'type'=>$join['type']))->save($post);

            $row1 = M('repair_joininfo')->where(array('join_id'=>$join_id))->save($post);
            if($row1 !== false){
                return array('status'=>1,'msg'=>'加盟信息提交成功','result'=>$post);
            }else{
                return array('status'=>-1,'msg'=>'操作完成','result'=>$join_id);
            }
        }
        ...
        $post['user_id'] = $user_id;
        $post['add_time'] = time();
        $join_id = M('repair_join')->add($post);
        //如果设为默认地址
        $insert_id = DB::name('repair_join')->getLastInsID();
        //
        $post['join_id'] = $insert_id;

        if(!$join_id)
            return array('status'=>-1,'msg'=>'添加失败','result'=>'');

         $joininfo_id = M('repair_joininfo')->add($post);
        return array('status'=>1,'msg'=>'添加成功','result'=>$join_id);
    }
     ...
     * $action 标识 是否查询工程师信息
     * */
    public function  get_engineer_info($id,$action = 0){
         // 工程师好评平均值
       // $rank = Db::name('repair_evaluate')->field('user_rank')->where(['user_id' => $id])->select();
        $rank = M('repair_order')
            ->field('e.order_id,o.user_rank')
            ->alias('e')
            ->join('repair_evaluate o', 'e.order_id =o.order_id')
            ->where("e.engineer_id = $id")
            ->select();
        // 已完成服务总次数
        //$result['service_num'] = Db::name('repair_evaluate')->field('user_rank')->where(['user_id' => $id])->count();
        $result['service_num'] = M('repair_evaluate')
            ->field('e.user_rank')
            ->alias('e')
            ->join('repair_order o', 'e.order_id =o.order_id')
            ->where("o.engineer_id = $id")
            ->count();
        if ($rank){
            $rank_cout = count($rank);
            $rank_arr = array_column($rank,'user_rank');
            $result['accumulation'] = round(array_sum($rank_arr) / $rank_cout,1);
            $result['evaluation'] = floor(array_sum($rank_arr) / $rank_cout);
        } else {
        ...
        $where['type'] = 1;
        // 总收入
        $account_all = Db::name('account_log')->field('user_money')->where($where)->select();
        $account_all_arr = array_column($account_all,'user_money');
        $result['account_all'] = round(array_sum($account_all_arr),2);
        $where['change_time'] = ['BETWEEN',"$beginThismonth,$endThismonth"];

         // 当月收入
        $account_month = Db::name('account_log')->field('user_money')->where($where)->select();
        $account_month_arr = array_column($account_month,'user_money');
        $result['account_month'] = round(array_sum($account_month_arr),2);
        // 待处理订单数量
        $where = ' o.engineer_id = '.$id;
        $where .= C(strtoupper('WAITDO'));
        $result['wait'] = Db::name('repair_order')->alias('o')->where($where)->count();
        if ($action == 1) {
            $where = ['user_id' => $id, 'is_engineer'=>1, 'engineer_status'=>1];
            $result['engineer_info'] = Db::name('users')->where($where)->find();
        }
         return $result;
    }
