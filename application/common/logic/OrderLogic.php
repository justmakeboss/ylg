...
class OrderLogic
{
	...
	public function cancel_order($user_id,$order_id,$action_note='您取消了订单'){
        ...
        if($order['type'] == 2){
            ...
            if($order['pay_status'] == 0 && $order['type'] == 2){
            ...
            return array('status'=>-1,'msg'=>'该订单不支持取消','result'=>'');
        }
    }
     ...
     * @return array
     */
    public function cancel_server_order($user_id,$order_id,$action_note='您取消了订单'){
        $order = M('repair_order')->where(array('order_id'=>$order_id,'user_id'=>$user_id))->find();
        //检查是否未支付订单 已支付联系客服处理退款
        if(empty($order))
            return array('status'=>-1,'msg'=>'订单不存在','result'=>'');
        ...
        $row = M('repair_order')->where(array('order_id'=>$order_id,'user_id'=>$user_id))->save(array('order_status'=>5));
        ...
        M('order_action')->add($data);//订单操作记录
        if(!$row)
            return array('code'=>-1,'msg'=>'操作失败','result'=>'');
        return array('code'=>1,'msg'=>'操作成功','result'=>'');
	 ...
	 * @return array
	 */
	public function addOrder($user_id,$suppliers_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price=[],$user_note='',$pay_name='',$order_type = 0,$start_server_time = 0,$end_server_time = 0,$type)
	{
		...
		$data = array(
                ...
                'suppliers_id'     =>$suppliers_id, // 门店id
        ...
        if($suppliers_id){
            ...
            // 保证不会有重复订单号存在
            while(true){
                $repair_order_sn = 'SO'.date('YmdHis').rand(10,99); // 订单编号
                $order_sn_count = Db::name('repair_order')->where("order_sn = '$repair_order_sn'")->count();
                if($order_sn_count == 0)
                    break;
            }

            $repair_data = array(
                'order_sn'          =>$repair_order_sn, // 订单编号
                'user_id'           =>$user_id, // 用户id
                'order_buy_id'      =>$order['order_id'],
                'suppliers_id'      =>$suppliers_id, // 门店id
                'order_status'      =>1,// 订单状态（接单中，维修中，已完成，待评价，已关闭，已取消）
                'order_type'        =>$order_type, // 订单类型（0寄修，1上门，2到店）
                'order_state'       =>0, // 订单标识(0商品安装/1维修服务单)
                'pay_status'        =>0, // 支付状态0未支付1已支付
                'paid_price'        =>tpCache('basic.prepaid_price'),//'支付价格',
                'prepaid_price'     =>tpCache('basic.prepaid_price'), // 预付价格
                'total_price'       =>tpCache('basic.prepaid_price'),//'订单总价',
                'province'          =>$address['province'],//'省份id',
                'city'              =>$address['city'],//'城市id',
                'district'          =>$address['district'],//'县',
                ...
                'end_server_time'   =>$end_server_time,
                'add_time'          =>time(), // 下单时间
                'pay_name'          =>$pay_name,//支付方式
                'order_note'        =>$user_note, // 用户下单备注
            );
            $repair_order_id = Db::name('repair_order')->insertGetId($repair_data);
            // 记录订单操作日志
            $action_info = array(
                'order_id'        =>$repair_order_id,
                'action_user'     =>0,
                'action_note'     => '您提交商品预约安装服务订单，请等待系统确认',
                'status_desc'     =>'提交服务订单', //''
                'log_time'        =>time(),
            );
            M('repair_action')->insertGetId($action_info);
        }
		// 记录订单操作日志
		$action_info = array(
		...
		foreach($cartList as $key => $val)
		{
			...
			$data2['quota']        = $val['quota']; // 商品价  配额
			$data2['setmeal_id']        = $val['setmeal_id']; // 商品价  配额
		...
		//分销开关全局
		try{
			$distribut_switch = distributCache('role_info.switch');
			if($distribut_switch  == 1 && file_exists(APP_PATH.'common/logic/DistributPrizeLogic.php'))
			{
				$distributLogic = new \app\common\logic\DistributPrizeLogic();
				$distributLogic->setUserId($user_id);
				$distributLogic->setConfigCache('general_prize');
				$distributLogic->general_prize($order); // 生成分成记录
			}
		}catch(\Exception $e)
		{
        ...
        $wechat->sendTemplateMsgOnPayPlace($order);
        ...
        if(!$perpetual) { //  没有永久绑定上下级关系
            ...
            if ($level_state == 1) {
	            // 首次下单 永久绑定上下级关系
	            Db::name('users')->where(['user_id' => $user_id])->save(['perpetual'=>1]);
	            $wechat->sendTemplateMsgBindLeader($leader['first_leader'], $user_id);

            } 
        }


		//准备验证升级角色,此处是下订单的地方----------标记<QualificationLogic>
		//$qualificationLogic = new \app\common\logic\QualificationLogic();
		//$qualificationLogic->prepare_update_level($user_id,['order_id'=>$order['order_id']]);
		return array('status'=>1,'msg'=>'提交订单成功','result'=>$order['order_id'],'price'=>$data['total_amount']); // 返回新增的订单id
	}
    /*
     * 添加服务订单
     * **/
    public function addServerOrder($user_id,$plan_id,$suppliers_id,$order_type,$prepaid_price,$paid_price,$total_price,$username,$address,$phone,$start_server_time,$end_server_time,$shipping_name=null,$shipping_code=null,$order_note='',$order_state=1,$order_status=0,$pay_status=0)
    {
        $order_sn = null;
        // 保证不会有重复订单号存在
        while(true){
            $order_sn = 'SO'.date('YmdHis').rand(10,99); // 订单编号
            $order_sn_count = Db::name('repair_order')->where("order_sn = '$order_sn'")->count();
            if($order_sn_count == 0)
                break;
        }
        ...
        $data = array(
            ...
            'plan_id'          =>$plan_id,
            'suppliers_id'     =>$suppliers_id, // 门店id
            'order_status'     =>$order_status,// 订单状态（接单中，维修中，已完成，待评价，已关闭，已取消）
            'order_type'        =>$order_type, // 订单类型（0寄修，1上门，2到店）
            'order_state'        =>$order_state, // 订单标识(0商品安装/1维修服务单)
            'pay_status'        =>$pay_status, // 支付状态0未支付1已支付
            'paid_price'     =>$paid_price,//'支付价格',
            'prepaid_price'  => $prepaid_price, // 预付价格
            'total_price'     =>$total_price,//'订单总价',
            'address'         =>$address,
            'mobile'          =>$phone,
            'shipping_name'   =>$shipping_name, //物流名称
            'shipping_code'   => $shipping_code, //物流订单编号
            'consignee'       =>$username,
            'start_server_time'       =>$start_server_time,
            'end_server_time'       =>$end_server_time,
            'add_time'         =>time(), // 下单时间
            'order_note'        =>$order_note, // 用户下单备注
        );
        $order_id = Db::name('repair_order')->insertGetId($data);

        if(empty($order_id)){
            return array('status'=>-8,'msg'=>'添加订单失败','result'=>null);
        }
        $order_id = Db::name('repair_order')->getLastInsID();
        // 记录订单操作日志
        $action_info = array(
            'order_id'        =>$order_id,
            'action_user'     =>0,
            'action_note'     => '您提交了维修服务订单，请等待系统确认',
            'status_desc'     =>'提交维修订单', //''
            'log_time'        =>time(),
        );
        M('repair_action')->insertGetId($action_info);
        ...
        // 发送微信消息模板提醒
        $wechat = new \app\common\logic\WechatLogic;
        $wechat->sendTemplateMsgServiceOrderAdd($order);
        ...
        return array('status'=>'1','msg'=>'提交订单成功','result'=>$order_id,'price'=>$data['paid_price']); // 返回新增的订单id
    }
