...
class DistributLogic extends Model{
    ...
    public function rebateLog($order){
        ...
        $regrade    = tpCache('distribut.regrade'); //返佣级数0返一级1返二级2返三级
    ...
    public static function rebateDivide($user_id)
    {
        ...
        M('rebate_log')->where(['user_id'=>$user_id,'status'=>2])->chunk(20,function($user){
            ...
            if($maid_time == 2){
                ...
                $rebate = $maid_time ? 'pay_time' : 'delivery_time';
            }
            foreach($user as $log)
            {
                if( (time() - $log[$rebate]) >= distributCache('settlement.date') * 86400){
    ...
    function auto_confirm(){
        ...
        $switch = distributCache('role_info.switch');
        if($switch == 0)
            return false;
        $maid_time = distributCache('settlement.maid_time');
        if($maid_time == 2){
        ...
        $today_time = time();
        $distribut_date = distributCache('settlement.date');
        $distribut_time = $distribut_date * (60 * 60 * 24); // 计算天数 时间戳
        $rebate_log_arr = M('rebate_log')->where("status = 2 and ($today_time - $rebate) >  $distribut_time")->select();
