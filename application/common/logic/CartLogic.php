...
class CartLogic extends Model
{
     ...
     */
    //        liushuang
    public function setGoodsModel($goods_id,$setmeal = 0)
    {
        $goodsModel = new Goods();
        if($setmeal > 0){
                $this->goods = Db::name('goods_setmeal')
                    ->alias('s')
...
//        liushuang
        if($this->goods['type_id'] == 6 && $this->goods['status'] == 0){
            $price = $this->goods['trade_price'];
//            dump($price);exit;
            $buyGoods['goods_price'] = $price; //代理产品批发价
            $buyGoods['retail_price'] = $this->goods['goods_price']; //代理产品零售价格
            $buyGoods['member_goods_price'] = $price; //代理产品批发价
            $buyGoods['quota'] = $this->goods['quota']; //配额
            $buyGoods['goods_id'] = $this->goods['id']; //套餐ID
            $buyGoods['setmeal_id'] = $this->goods['goods_id']; //商品ID
        }
        if(empty($this->goods['type_id'])){
            $buyGoods['shop_integral'] = $this->goods['cost_price']; // 消费积分
        }
//        elseif($this->goods['type_id'] == 6 && $this->goods['status'] == 1){
//            $buyGoods['goods_price'] = $this->goods['price']; //零售价
//            $buyGoods['member_goods_price'] = $this->goods['price']; //零售价
//        }
//        liushuang

        $Distribut = new DistributPrizeLogic();
        $Distribut->setUserId($this->user_id);
        ...
        if(empty($this->specGoodsPrice)){
            ...
            // 立即购买折扣价
            $buyGoods['member_goods_price'] = $Distribut->discount_prize($this->goods);
            // END 7-12
        }else{
            $this->specGoodsPrice['type_id'] = $this->goods['type_id'];
            // 立即购买折扣价
            $buyGoods['member_goods_price'] = $Distribut->discount_prize($this->specGoodsPrice,'price');
            // END 7-12
