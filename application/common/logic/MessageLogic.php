...
class MessageLogic extends Model
{
    ...
    /**
     * 获取用户的系统消息
     * @param type $category|消息类型
     * @return array
     */
    public function getUserMessageNotice($category=0,$state=0)
    {
        $this->checkPublicMessage();
        $user_info = session('user');
        $user_system_message_no_read_where = array(
        ...
        $user_system_message_no_read = Db::name('user_message')
            ->alias('um')
            ->field('um.rec_id,um.user_id,um.category,um.message_id,um.status,m.send_time,m.type,m.message,m.data,m.common_id')
            ->join('__MESSAGE__ m','um.message_id = m.message_id','LEFT')
            ->where($user_system_message_no_read_where)
            ->select();
        return $user_system_message_no_read;
    ...
    public function getAnnouncement()
    {
        ...
        $announcement = Db::name('user_message')
            ->alias('um')
            ->field('um.rec_id,um.user_id,um.category,um.message_id,um.status,m.send_time,m.type,m.message,m.data,m.common_id,a.title,a.description,a.add_time')
            ->join('__MESSAGE__ m','um.message_id = m.message_id','LEFT')
            ->join('__ARTICLE__ a','m.common_id = a.article_id','LEFT')
