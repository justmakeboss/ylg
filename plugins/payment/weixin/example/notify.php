...
class PayNotifyCallBack extends WxPayNotify
{
	...
	{                       
        ...
        }else if(stripos($order_sn, 'SO') !== false){
		    // 服务订单的支付金额
            $order_amount = M('repair_order')->where(['order_sn'=>"$order_sn"])->value('paid_price');
        }else{
		    // 普通订单的支付金额
            $order_info = M('order')->field('order_amount,order_id')->where(['order_sn'=>"$order_sn"])->find();
            $order_amount = $order_info['order_amount'];
            $server_order_amount = M('repair_order')->where(['order_buy_id'=>$order_amount['order_id']])->value('paid_price');
            if (!empty($server_order_amount))$order_amount += $server_order_amount;
        }
        if((string)($order_amount * 100) != (string)$data['total_fee'])
            return false; //验证失败

        if(stripos($order_sn, 'SO') !== false){
            update_server_pay_status($order_sn, array('transaction_id'=>$data["transaction_id"]));
        }else{
